  ...
  var specialStartingChars = ["^", "@", "#", "%"];
	...
	sunlight.registerLanguage("clojure", {
      ...
      function(context) {
        ...
        if(specialStartingChars.indexOf(value) === -1 || context.reader.peek() === "\"") {
          return null;
        }
				return context.createToken("named-ident", value, context.reader.getLine(), context.reader.getColumn());
      }
		],
      ...
      numberParser: function(context) {
        ...
        var match = /^-?\d+(.\d+)?[NM]?\b/.exec(context.reader.substring())[0];
        if (match) {
          context.reader.read(match.length - 1);
          return context.createToken("number", match, context.reader.getLine(), context.reader.getColumn());
        }
